/*
 * pedroz-20251107.js - 20251107 - v1.34.2
 * moduleRaid v6
...
window.WWebJS.checkNumberStatus = async (id) => {
  try {
    ...
    // const isValid = await Store.QueryExist(wid);
...
window.WWebJS.sendMessageToId = async (chatId, message) => {
  try {
    ...
    // const chatWid = window.Store.WidFactory.createWid(chatId);
    // const chat = await window.Store.FindOrCreateChat.findOrCreateLatestChat(
    //   chatWid
    // );
    // const chat = await window.Store.FindOrCreateChat.findExistingChat(chatWid);
...
window.WWebJS.sendMediaMessage = async (
  ...
  try {
    ...
    const msg = await window.WPP.chat.sendFileMessage(
      chatId,
      base64,
      caption,
      filename,
...
window.WWebJS.getAllGroups = async (done) => {
  ...
  const groups = await window.Store.GroupMetadata.getModelsArray().map(
    (group) => ({
      id: group.id,
      name: group.subject,
    })
...
window.WWebJS.getGroupParticipantIDs = async (groupId, done) => {
  ...
  const output = await a.participants._models.map((data) => ({
    phone: data.contact.phoneNumber.user,
    name:
      data.contact.name ||
...
window.WAPI.isBusiness = function () {
  return Store.Conn.isSMB;
};

window.WAPI.sendMessageToID1 = async (chatId, message) => {
  return await WWebJS.sendMessageToId(chatId, message);
};

window.WAPI.sendImageToID = async (base64, chatId, filename, caption) => {
  return await WWebJS.sendMediaMessage(base64, chatId, filename, caption);
};

window.WAPI.checkNumberStatus = async (chatId) => {
  const numberExist = await window.WWebJS.checkNumberStatus(chatId);
